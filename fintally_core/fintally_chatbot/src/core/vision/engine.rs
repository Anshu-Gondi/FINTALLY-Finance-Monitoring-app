...
impl VisionEngine {
    ...
    /// Flexible constructor allowing custom model paths without rigid transformer schema locks
    pub fn from_model_dir<P: AsRef<Path>>(model_dir: P) -> Result<Self> {
        ...
        // Flexible EOS token detection across different model families
        ...
        // Load Config generically using serde_json::Value to avoid schema mismatch crashes
        ...
        if !weights_path.exists() {
            bail!("Model weights file not found at {:?}", weights_path);
        }
    ...
    /// Process input with default target dimensions (1920x1080, 3 channels)
    pub fn process_input(&mut self, input: DocumentInput, prompt: &str) -> Result<String> {
        self.process_input_with_dims(input, prompt, 1920, 1080, 3)
    }

    /// Process input with explicitly specified target dimensions
    pub fn process_input_with_dims(
        ...
        // Preprocess document through high-performance C++ engine via FFI
        ...
        // Construct Candle Tensor from zero-copy C++ preprocessed pixels
        let image_tensor = Tensor::from_slice(pixel_data, (height, width, channels), &self.device)?;

        // Convert NHWC to NCHW standard vision format and normalize [0, 1]
