...
impl RagService {
    ...
    pub fn new(storage_path: &str, embedding_model_path: &str) -> Result<Self, AppError> {
        ...
        let absolute_model = std::fs
            ::canonicalize(&model_dir)
            .map_err(|e| {
        ...
        println!("🚀 [usearch-bridge] Passing sanitised path to native engine: {}", clean_model_path);
        ...
        if !config_file_path.exists() {
            return Err(
                AppError::InferenceError(
                    format!("CRITICAL: Rust confirmed 'config.json' is physically MISSING from the directory: {}", clean_model_path)
                )
            );
        }
    ...
    pub async fn ingest_document(&self, file_path: &str) -> Result<(), AppError> {
        ...
        match ext.as_str() {
            "txt" | "json" | "csv" => {
                let mut file = File::open(path_obj).map_err(|e|
                    AppError::InferenceError(format!("File read error on open: {e}"))
                )?;
            ...
            "pdf" | "docx" => {
                println!(
                    "[RAG-SERVICE] Handing multi-stage layout extraction to native filters for: {}",
                    path_obj.display()
                );
            }
            _ => {
                return Err(
                    AppError::InferenceError(format!("Unsupported document format: .{ext}"))
                );
            }
        }
        ...
        engine_guard
            ...
            .map_err(|e|
                AppError::InferenceError(format!("Native core embedding extraction error: {e}"))
            )?;
    ...
    pub async fn search_knowledge(&self, query_text: &str, limit: usize) -> String {
        ...
        match engine_guard.query(query_text, Some(limit), Some(0.0), None) {
            Ok(query_response) => query_response.context_block.into_string(),
            Err(e) => {
    ...
    async fn build_secure_drive_client(
        ...
        DriveHub<google_drive3::hyper_rustls::HttpsConnector<google_drive3::hyper_util::client::legacy::connect::HttpConnector>>,
        AppError
    > {
        ...
        let app_secret = yup_oauth2
            ::read_application_secret(active_secret_path).await
            .map_err(|e|
                AppError::InferenceError(
                    format!("Failed reading application client_secret.json layout: {e}")
                )
            )?;
        ...
        let client = hyper_util::client::legacy::Client
            ::builder(hyper_util::rt::TokioExecutor::new())
    ...
    pub async fn sync_and_reindex_trusted_sources(&self) -> Result<(), AppError> {
        ...
        let folder_query =
            "name = 'Llm docs' and mimeType = 'application/vnd.google-apps.folder' and trashed = false";
        ...
        let (_, items_results) = hub
            ...
            .map_err(|e|
                AppError::InferenceError(
                    format!("Failed reading remote directory asset trees: {e}")
                )
            )?;
        ...
        for file_meta in discovered_assets {
            ...
            let (stream_response, _) = hub
                ...
                .map_err(|e|
                    AppError::InferenceError(
                        format!("Network download processing fault on object {item_name}: {e}")
                    )
                )?;
            ...
            let raw_bytes = stream_response
                ...
                .map_err(|e| {
                    AppError::InferenceError(
                        format!("Failed formatting binary chunk sequences: {e}")
                    )
                })?
                .to_bytes();
