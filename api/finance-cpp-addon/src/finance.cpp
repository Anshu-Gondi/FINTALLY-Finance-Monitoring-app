...
namespace finance
{
    ...
    // {
    //   emi: BigInt,
    //   principalPaid: BigInt64Array,
    //   interestPaid: BigInt64Array,
    //   balance: BigInt64Array
    // }

    Napi::Value CalculateEMIAmortization(const Napi::CallbackInfo &info)
    {
        Napi::Env env = info.Env();

        if (info.Length() != 3)
        {
            Napi::TypeError::New(env, "Expected principal, rate, months")
                .ThrowAsJavaScriptException();
            return env.Null();
        }

        // ✅ BigInt lossless conversion
        bool lossless;
        int64_t principal = info[0].As<Napi::BigInt>().Int64Value(&lossless);
        if (!lossless)
        {
            Napi::RangeError::New(env, "BigInt too large to convert losslessly")
                .ThrowAsJavaScriptException();
            return env.Null();
        }

        double annualRate = info[1].As<Napi::Number>().DoubleValue();
        int32_t months = info[2].As<Napi::Number>().Int32Value();

        if (principal <= 0 || months <= 0)
        {
            Napi::RangeError::New(env, "Invalid EMI inputs")
                .ThrowAsJavaScriptException();
            return env.Null();
        }
        ...
        {
            double p = std::pow(1.0 + r, months);
            emi = static_cast<int64_t>(
                std::llround((principal * r * p) / (p - 1.0)));
        }

        Napi::BigInt64Array principalPaid = Napi::BigInt64Array::New(env, months);
        Napi::BigInt64Array interestPaid = Napi::BigInt64Array::New(env, months);
        Napi::BigInt64Array balanceArr = Napi::BigInt64Array::New(env, months);

        auto *pp = principalPaid.Data();
        auto *ip = interestPaid.Data();
        auto *bal = balanceArr.Data();

        int64_t balance = principal;

        for (int i = 0; i < months; i++)
        {
            int64_t interest = (r == 0.0)
                                   ? 0
                                   : static_cast<int64_t>(std::llround(balance * r));

            int64_t principalComponent = emi - interest;
            balance -= principalComponent;
            if (balance < 0)
                balance = 0;

            pp[i] = principalComponent;
            ip[i] = interest;
            bal[i] = balance;
        }

        Napi::Object result = Napi::Object::New(env);
        result.Set("emi", Napi::BigInt::New(env, emi));
        result.Set("principalPaid", principalPaid);
        result.Set("interestPaid", interestPaid);
        result.Set("balance", balanceArr);

        return result;
    }

    // Inputs
    // emis[]        BigInt64Array   // EMI amount
    // startMonth[]  Int32Array     // offset (0 = this month)
    // tenure[]      Int32Array

    // Output
    // monthlyTotal[] BigInt64Array

    Napi::Value CalculateMultiEmiMonthlyAggregation(const Napi::CallbackInfo &info)
    {
        Napi::Env env = info.Env();

        if (info.Length() != 3)
        {
            Napi::TypeError::New(env, "Expected emis, startMonth, tenure")
                .ThrowAsJavaScriptException();
            return env.Null();
        }

        auto emis = info[0].As<Napi::BigInt64Array>();
        auto start = info[1].As<Napi::Int32Array>();
        auto tenure = info[2].As<Napi::Int32Array>();

        size_t n = emis.ElementLength();
        if (!ValidateSameLength(n, start.ElementLength(), env) ||
            !ValidateSameLength(n, tenure.ElementLength(), env))
        {
            return env.Null();
        }
        ...
        for (size_t i = 0; i < n; i++)
        {
            maxMonth = std::max(maxMonth, S[i] + T[i]);
        }

        Napi::BigInt64Array monthlyTotal =
            Napi::BigInt64Array::New(env, maxMonth);

        auto *out = monthlyTotal.Data();
    ...
    Napi::Object Init(Napi::Env env, Napi::Object exports)
    {
        ...
        exports.Set("emiAmortization",
                    Napi::Function::New(env, CalculateEMIAmortization));

        exports.Set("multiEmiMonthlyAggregation",
                    Napi::Function::New(env, CalculateMultiEmiMonthlyAggregation));
