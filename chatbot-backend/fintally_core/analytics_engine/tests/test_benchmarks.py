...
def generate_sample_data(n=1000):
    ...
    categories = [random.choice(
        ["food", "rent", "travel", "salary"]) for _ in range(n)]
...
def test_emi_survivability_score():
    score, label = rust_backend.emi_survivability_score(
        monthly_income=100_000, monthly_emi=15_000)
...
def test_emi_monthly_pressure():
    dates = ["2024-01-10T00:00:00Z",
             "2024-01-15T00:00:00Z", "2024-02-10T00:00:00Z"]
...
def test_detect_recurring_anomalies():
    dates = ["2024-01-01T00:00:00Z",
             "2024-01-31T00:00:00Z", "2024-03-10T00:00:00Z"]
    prices = [-1000, -1000, -3000]

    res = rust_backend.detect_recurring_anomalies(
        dates, prices, expected_frequency_days=30, tolerance_pct=0.3)
    assert isinstance(res, list)
...
def benchmark(func, *args, label=None, max_time=1.0):
    ...
    if label:
        print(
            f"✅ {label}: {len(res) if isinstance(res, list) else res} in {elapsed:.4f}s")
...
def test_aggregate_by_interval_perf():
    ...
    benchmark(rust_backend.aggregate_by_interval, dates, prices,
              15, label="aggregate_by_interval", max_time=10.0)


def test_aggregate_by_category_perf():
    _, prices, cats = generate_sample_data(1_000_000)
    benchmark(rust_backend.aggregate_by_category, cats, prices,
              10, label="aggregate_by_category", max_time=5.0)


def test_aggregate_by_day_perf():
    dates, prices, _ = generate_sample_data(1_000_000)
    benchmark(rust_backend.aggregate_by_day, dates, prices,
              None, label="aggregate_by_day", max_time=10.0)


def test_aggregate_by_month_perf():
    dates, prices, _ = generate_sample_data(1_000_000)
    benchmark(rust_backend.aggregate_by_month, dates, prices,
              label="aggregate_by_month", max_time=10.0)


def test_find_min_max_perf():
    dates, prices, _ = generate_sample_data(1_000_000)
    benchmark(rust_backend.find_min_max, dates, prices,
              label="find_min_max", max_time=5.0)


def test_aggregate_trend_monthly_perf():
    dates, prices, _ = generate_sample_data(1_000_000)
    benchmark(rust_backend.aggregate_trend, dates, prices, "monthly",
              label="aggregate_trend_monthly", max_time=15.0)


def test_aggregate_trend_weekly_perf():
    dates, prices, _ = generate_sample_data(1_000_000)
    benchmark(rust_backend.aggregate_trend, dates, prices, "weekly",
              label="aggregate_trend_weekly", max_time=15.0)
